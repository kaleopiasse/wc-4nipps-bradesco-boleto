    ...
    public function generate_bank_slip( $order ) {
        ...
        $new_data = array(
            ...
            'boleto' => array(
                ...
                'carteira' => $this->get_option('wallet'),
        ...
        if (intval(curl_getinfo($ch)['http_code']) != 201) {
            $http_code_error = intval(curl_getinfo($ch)['http_code']);
        }
        ...
        // rise a error
        if (isset($error_msg) || isset($http_code_error) || intval($result_decoded->status->codigo) != 0) {
            return;
        }
